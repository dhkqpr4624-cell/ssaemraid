...
export async function POST(req: NextRequest) {
  ...
  const response=await fetch(`${base}/api/raid-rewards`,{method:"POST",cache:"no-store",headers:{"content-type":"application/json","x-schoolraid-secret":secret},body:JSON.stringify({action:"issue",outcome:body.outcome,sourceKey:body.sourceKey,roomCode:body.roomCode})});
