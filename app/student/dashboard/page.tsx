...
import { getBossBattleSession, hasBossParticipant } from '@/lib/boss-battle';
import { AvatarRenderer } from '@/components/avatar/AvatarRenderer';
import type { Student, ClassRoom, Quiz, StudentSession, StudentQuizResult } from '@/lib/types';
import { canRetryQuizNow } from '@/lib/retry-attempts';
...
function getQuizStatus(quiz: Quiz, result: StudentQuizResult | null): { status: QuizStatus; result?: StudentQuizResult } {
  ...
  if (canRetryQuiz(quiz, result) && (rewardClaimed || !hasRewardToChoose)) return { status: 'retry_available', result };
...
export default function StudentDashboardPage() {
  ...
  const [bossBattleSession, setBossBattleSession] = useState<any>(null);
  const [bossBattleJoined, setBossBattleJoined] = useState(false);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (studentRes.success && studentRes.student && classData) {
        ...
        const [liveSessions, bossSession] = await Promise.all([getLiveSessionsForClass(savedSession.classCode), getBossBattleSession(savedSession.classCode)]);
        setLiveSessionsByQuizId(Object.fromEntries(liveSessions.map((live: any) => [live.quizId, live])));
        const visibleBossSession = bossSession?.status === "reward_ready" ? null : bossSession;
        setBossBattleSession(visibleBossSession);
        setBossBattleJoined(visibleBossSession ? await hasBossParticipant(savedSession.classCode, visibleBossSession.id, studentRes.student.id) : false);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      const [liveSessions, bossSession] = await Promise.all([getLiveSessionsForClass(session.classCode), getBossBattleSession(session.classCode)]);
      setLiveSessionsByQuizId(Object.fromEntries(liveSessions.map((live: any) => [live.quizId, live])));
      const visibleBossSession = bossSession?.status === "reward_ready" ? null : bossSession;
      setBossBattleSession(visibleBossSession);
      if (visibleBossSession && student?.id) setBossBattleJoined(await hasBossParticipant(session.classCode, visibleBossSession.id, student.id));
      else setBossBattleJoined(false);
    }, 3000);
    return () => clearInterval(timer);
  }, [session?.classCode, student?.id]);
  ...
  return (
    <main className="min-h-screen p-4 pb-24 md:p-8 bg-muted/20">
      <div className="max-w-2xl mx-auto">
        ...
        {bossBattleSession && bossBattleSession.status !== 'ended' && (
          <Card className="overflow-hidden border-2 border-amber-400 bg-gradient-to-r from-slate-950 to-amber-950 text-white shadow-lg">
            <CardContent className="p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 text-xl font-black text-amber-200"><Swords className="w-6 h-6"/>타락한 해태 보스전</div>
                <p className="text-sm text-amber-50/80 mt-1">문제 {bossBattleSession.questionCount}개 · 문제당 {bossBattleSession.timeLimitSeconds}초</p>
              </div>
              <Link href={`/student/boss-battle?code=${session.classCode}`}>
                <button className="relative min-w-[240px] h-[66px] px-8 font-black text-lg text-amber-950 hover:scale-[1.02] active:scale-95 transition-transform bg-center bg-[length:100%_100%]" style={{backgroundImage:"url('/boss-battle/button-panel.png')"}}>
                  {bossBattleJoined ? '보스전 이어서 참여하기' : '보스전 참여하기'}
                </button>
              </Link>
            </CardContent>
          </Card>
