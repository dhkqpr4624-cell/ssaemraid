...
export default function StudentLiveQuizPage() {
  ...
  const currentQuestionIndexKey = String(session?.currentQuestionIndex ?? 0);
  const submittedAnswerFromSession = attendanceNumber
    ? ((session as any)?.studentAnswers?.[currentQuestionIndexKey] || {})[attendanceNumber]
    : null;
  const submittedAnswerForCurrentQuestion = submittedAnswerFromSession
    || (Number(lastSubmittedAnswer?.questionIndex) === Number(session?.currentQuestionIndex) ? lastSubmittedAnswer.answer : null);
  const isShowingAnswer = session?.status === 'showing_answer';
  const isSubmittedAnswerCorrect = currentQuestion && submittedAnswerForCurrentQuestion
    ? gradeAnswer(currentQuestion, submittedAnswerForCurrentQuestion).earnedPoints > 0
    : false;
  const selectedOptionsForFeedback = Array.isArray(submittedAnswerForCurrentQuestion?.selectedOptions)
    ? submittedAnswerForCurrentQuestion.selectedOptions
    : [];
  const shortAnswerForFeedback = submittedAnswerForCurrentQuestion?.textAnswer || '';

  const getOptionButtonClassName = (idx: number) => {
    const isSelected = Array.isArray(selectedAnswer) && selectedAnswer.includes(idx);
    const wasSelected = selectedOptionsForFeedback.map(Number).includes(idx);
    if (isShowingAnswer && wasSelected) {
      return isSubmittedAnswerCorrect
        ? 'border-green-500 bg-green-50 ring-2 ring-green-200'
        : 'border-red-500 bg-red-50 ring-2 ring-red-200';
    }
    return isSelected ? 'border-purple-500 bg-purple-50 ring-2 ring-purple-200' : 'border-gray-100 bg-gray-50/50 hover:bg-gray-50';
  };

  const getShortAnswerClassName = () => {
    if (isShowingAnswer && submittedAnswerForCurrentQuestion) {
      return isSubmittedAnswerCorrect
        ? 'w-full p-4 border-2 rounded-xl focus:outline-none bg-green-50 border-green-500 ring-2 ring-green-200'
        : 'w-full p-4 border-2 rounded-xl focus:outline-none bg-red-50 border-red-500 ring-2 ring-red-200';
    }
    return 'w-full p-4 border-2 border-gray-100 rounded-xl focus:border-purple-500 focus:outline-none bg-gray-50/50';
  };

  const handleExitToDashboard = () => {
    router.push(code ? `/student/dashboard?code=${code}` : '/student/dashboard');
  };

  const mergeCurrentQuestionAnswers = async (liveSession: any) => {
    if (!code || !quizId || !liveSession) return liveSession;
    const effectiveQuizId = quiz?.id || quizId;
    const questionIndex = Number(liveSession.currentQuestionIndex ?? 0);
    try {
      const answersForQuestion = await getLiveAnswers(code, effectiveQuizId, questionIndex);
      if (!answersForQuestion || Object.keys(answersForQuestion).length === 0) return liveSession;
      const questionKey = String(questionIndex);
      const studentAnswers = { ...((liveSession as any).studentAnswers || {}) };
      studentAnswers[questionKey] = { ...(studentAnswers[questionKey] || {}), ...(answersForQuestion || {}) };
      return {
        ...liveSession,
        studentAnswers,
        submittedStudents: Array.from(new Set([
          ...((liveSession.submittedStudents || []).map(String)),
          ...Object.keys(answersForQuestion || {}),
        ])),
      };
    } catch (err) {
      console.warn('실시간 학생 답안 동기화 실패:', err);
      return liveSession;
    }
  };
  ...
  const loadSession = async () => {
    ...
    if (latest) {
      const hydrated = await mergeCurrentQuestionAnswers(latest as any);
      setSession(hydrated as any);
    }
  };
    ...
    const loadData = async () => {
      try {
        ...
        if (initialSession && (initialSession as any).isTeacherLiveOpen === true && studentData && quizData) {
          ...
          const questionIndex = Number((initialSession as any).currentQuestionIndex ?? 0);
          let hydratedInitialSession: any = initialSession;
          try {
            const answersForQuestion = await getLiveAnswers(code, quizData.id, questionIndex);
            if (answersForQuestion && Object.keys(answersForQuestion).length > 0) {
              const questionKey = String(questionIndex);
              const studentAnswers = { ...((initialSession as any).studentAnswers || {}) };
              studentAnswers[questionKey] = { ...(studentAnswers[questionKey] || {}), ...(answersForQuestion || {}) };
              hydratedInitialSession = {
                ...(initialSession as any),
                studentAnswers,
                submittedStudents: Array.from(new Set([
                  ...(((initialSession as any).submittedStudents || []).map(String)),
                  ...Object.keys(answersForQuestion || {}),
                ])),
              };
            }
          } catch (err) {
            console.warn('초기 실시간 학생 답안 동기화 실패:', err);
          }
          setSession(hydratedInitialSession as any);
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (!attendanceNumber || submitted) return;
    const answers = (session as any)?.studentAnswers?.[String(session?.currentQuestionIndex ?? 0)] || {};
    if (answers[attendanceNumber]) setSubmitted(true);
  }, [session?.studentAnswers, session?.currentQuestionIndex, attendanceNumber, submitted]);
      ...
      if (response.success) {
        if (typeof window !== "undefined") {
          sessionStorage.setItem(
            `ssaemquest_reward_access_${quiz.id}_${student.id}_${attemptNumber}`,
            String(Date.now()),
          );
        }
  ...
  if (!session || session.status === 'waiting') {
    return (
      <main className="min-h-screen p-4 md:p-8 bg-gradient-to-b from-purple-50 to-pink-50 flex items-center justify-center relative">
        <Button variant="ghost" className="absolute left-4 top-4 gap-2" onClick={handleExitToDashboard}>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-6 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Button variant="ghost" size="sm" className="gap-1 shrink-0 px-2" onClick={handleExitToDashboard}>
              <ArrowLeft className="w-4 h-4" />
              나가기
            </Button>
          ...
          <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {currentQuestion.options.map((option, idx) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 text-left transition-all ${getOptionButtonClassName(idx)}`}
                  >
            ...
            {currentQuestion.type === 'short' && (
              <input
                type="text"
                disabled={submitted || session.status !== 'question_active'}
                value={isShowingAnswer && submittedAnswerForCurrentQuestion ? shortAnswerForFeedback : (typeof selectedAnswer === 'string' ? selectedAnswer : '')}
                onChange={e => setSelectedAnswer(e.target.value)}
                placeholder="답을 입력하세요..."
                className={getShortAnswerClassName()}
              />
            )}
          </CardContent>
        </Card>
        ...
        {submitted ? (
          isShowingAnswer && submittedAnswerForCurrentQuestion ? (
            <div className={`p-4 rounded-xl border-2 text-center ${isSubmittedAnswerCorrect ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
              <p className={`font-bold ${isSubmittedAnswerCorrect ? 'text-green-700' : 'text-red-700'}`}>
                {isSubmittedAnswerCorrect ? '정답입니다! 선생님의 다음 지시를 기다려주세요.' : '아쉽지만 오답입니다. 선생님의 다음 지시를 기다려주세요.'}
              </p>
            </div>
          ) : (
