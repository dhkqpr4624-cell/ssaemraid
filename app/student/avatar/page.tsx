...
// 해제 가능 슬롯
const REMOVABLE_SLOTS: AvatarSlot[] = ['hair', 'headAccessory', 'eyebrow', 'mouth'];
...
function ItemIcon({ item, size = 48, eyeColor, hairColor }: { item: Item; size?: number; eyeColor?: string; hairColor?: string }) {
  if (item.iconUrl) {
    return <img src={item.iconUrl} alt={item.name} className="object-contain rounded" style={{ width: size, height: size }} />;
  }
  if (item.imageUrl) {
...
function AvatarCustomizationContent() {
  ...
  const filteredItems = selectedSlot === 'all'
    ? avatarItems
    : avatarItems.filter(item => item.slot === selectedSlot || item.occupiesSlots?.includes(selectedSlot as AvatarSlot));
  ...
  const handleEquip = (item: Item) => {
    if (!item.slot) return;
    const targetSlots = item.occupiesSlots?.length ? item.occupiesSlots : [item.slot];
    setCurrentAvatar(prev => {
      const nextEquipped = { ...prev!.equipped };

      // 상하의 통합 아이템처럼 여러 슬롯을 차지하는 아이템과 충돌하면,
      // 겹치는 기존 아이템의 전체 점유 슬롯을 먼저 비웁니다.
      targetSlots.forEach(slot => {
        const previousItemId = nextEquipped[slot];
        const previousItem = student?.items?.find(candidate => candidate.id === previousItemId);
        const previousSlots = previousItem?.occupiesSlots?.length ? previousItem.occupiesSlots : [slot];
        previousSlots.forEach(previousSlot => {
          (nextEquipped as any)[previousSlot] = null;
        });
      });

      targetSlots.forEach(slot => {
        nextEquipped[slot] = item.id;
      });
      return {
        ...prev!,
  ...
  const handleUnequip = (slot: AvatarSlot) => {
    ...
    setCurrentAvatar(prev => {
      ...
      const equippedItemId = newEquipped[slot];
      const equippedItem = student?.items?.find(item => item.id === equippedItemId);
      const targetSlots = equippedItem?.occupiesSlots?.length ? equippedItem.occupiesSlots : [slot];
      ...
      targetSlots.forEach(targetSlot => {
        (newEquipped as any)[targetSlot] = null;
      });
      return {
        ...prev!,
  ...
  const isEquipped = (item: Item): boolean => {
    ...
    const targetSlots = item.occupiesSlots?.length ? item.occupiesSlots : [item.slot];
    return targetSlots.every(slot => currentAvatar?.equipped?.[slot as AvatarSlot] === item.id);
  };
      ...
      >
        ...
        <div className="space-y-0.5">
          ...
          <Badge variant="outline" className="text-[10px] px-1 py-0">
            {item.occupiesSlots?.length
              ? item.occupiesSlots.map(slot => SLOT_LABELS[slot] || slot).join('·')
            ...
            <Card className="sticky top-4">
              ...
              <CardContent className="flex flex-col items-center gap-4">
                ...
                <div className="w-full text-xs text-muted-foreground space-y-1">
                  {(['hair', 'headAccessory', 'eyes', 'eyebrow', 'mouth'] as AvatarSlot[]).map(slot => {
