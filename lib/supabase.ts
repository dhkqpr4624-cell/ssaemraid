...
import {
  ...
  ensureDefaultAvatarState,
} from "./default-avatar-items";
...
try {
  if (supabaseUrl && supabaseAnonKey) {
    ...
    if (typeof window !== "undefined") {
      console.warn(
        "[Supabase] Using dummy client - environment variables not set",
      );
    }
  }
} catch (err) {
...
function isDuplicateRestrictedInventoryItem(item: any): boolean {
  // 가구(room)는 여러 개 보유/배치 가능. room이 아닌 장착/기타 보상만 중복 방지 대상으로 둡니다.
...
function dedupeInventoryItems(items: any[] = []): any[] {
  ...
  for (const item of items || []) {
    ...
    if (isDuplicateRestrictedInventoryItem(item) && key) {
...
export async function createSupabaseClass(
  className: string,
  ...
  try {
    ...
    console.log("[Supabase] Creating class:", {
      classCode,
      className,
      studentCount,
    });
    ...
    const students = Array.from({ length: studentCount }, (_, i) => ({
      ...
      avatar_state: ensureDefaultAvatarState(DEFAULT_AVATAR_STATE as any),
...
export async function createSupabaseStudents(
  classCode: string,
  studentCount: number,
) {
  try {
...
// 2. 학생 관련
export async function getSupabaseStudent(
  classCode: string,
...
function normalizeRewardOptionsForStorage(
  rewards: any[] | undefined | null,
): QuizRewardOption[] {
...
function getRetryRewardModeFromSettings(
  settings: any,
): "same" | "different" | "none" {
  return (
...
function getRewardOptionsForAttemptFromQuizRow(
  quizRow: any,
...
function buildQuizPayload(quiz: Quiz) {
  ...
  return {
    ...
    settings: {
      ...
      retryQuestionMode:
        quiz.retryQuestionMode || quiz.retryOptions?.questionMode || "same",
      retryRewardMode:
        quiz.retryRewardMode || quiz.retryOptions?.rewardMode || "same",
      retryRewardOptions: normalizeRewardOptionsForStorage(
        quiz.retryRewardOptions,
      ),
      maxRetryAttempts: getEffectiveMaxRetryAttempts(quiz as any),
      retryOptions: {
        ...
        rewardMode:
          quiz.retryRewardMode || quiz.retryOptions?.rewardMode || "same",
        maxAttempts: getEffectiveMaxRetryAttempts(quiz as any),
      },
...
export function normalizeQuizResult(data: any): StudentQuizResult {
  ...
  return {
    ...
    selectedRewardOptionId:
      data.selected_reward_option_id || data.selected_reward || undefined,
...
export async function submitSupabaseQuizResult(result: StudentQuizResult) {
  try {
    ...
    // 클라이언트 상태와 무관하게 서버 저장 직전에 회차와 재도전 한도를 다시 검증합니다.
    const [{ data: quizRow, error: quizError }, { data: latestRows, error: latestError }] = await Promise.all([
      supabase.from("quizzes").select("allow_retry,settings").eq("quiz_id", result.quizId).single(),
      supabase
        .from("quiz_results")
    ...
    const latestAttempt = Number(latestRows?.[0]?.attempt_number || 0);
    const retryEnabled = Boolean(
      quizRow?.allow_retry ??
        quizRow?.settings?.allowRetry ??
        quizRow?.settings?.retryOptions?.enabled,
    );
    const maxRetries = getEffectiveMaxRetryAttempts({
      allowRetry: retryEnabled,
      settings: quizRow?.settings || {},
    });
    const maxTotalAttempts = 1 + maxRetries;
    ...
    if (latestAttempt > 0) {
      if (!retryEnabled) {
        return { success: false, error: "재도전이 허용되지 않은 퀴즈입니다." };
      }
      if (attemptNumber !== latestAttempt + 1) {
    ...
    const payload = {
      ...
      reward_claimed_per_attempt: result.rewardClaimedPerAttempt || {
        [attemptNumber]: false,
      },
...
export async function getSupabaseQuizResult(quizId: string, studentId: string) {
  try {
    ...
    if (!data) {
      console.log(
        "[Supabase] getSupabaseQuizResult: No result found (normal state)",
      );
      return null;
    }
...
export async function claimSupabaseQuizReward(
  ...
  try {
    ...
    const claimedMap: Record<string, boolean> =
      (quizResult.rewardClaimedPerAttempt as any) || {};
    const alreadyClaimed =
      !!quizResult.selectedRewardOptionId || !!claimedMap[key];
    ...
    if (quizResult.gradingStatus === "pending_review") {
      return {
        success: false,
        error: "아직 채점 대기 중이라 보상을 받을 수 없습니다.",
      };
    }
    ...
    if (quizError) throw quizError;
    if (!quizRow)
      return { success: false, error: "퀴즈 정보를 찾을 수 없습니다." };

    const rewardOptions = getRewardOptionsForAttemptFromQuizRow(
      quizRow,
    ...
    const eligibleIds = (quizResult.eligibleRewardOptionIds || []).map((id) =>
      String(id),
    );
    const isEligible =
    ...
    if (!isEligible) {
      return {
        success: false,
        error: "점수가 부족해서 이 보상을 받을 수 없습니다.",
      };
    }
    ...
    const item: Item & Record<string, any> =
      reward.itemType === "room"
        ...
        : {
            ...
            imageUrl: spriteUrl,
            hairLayerMode:
              (reward as any).hairLayerMode || catalogItem?.hairLayerMode,
            tintMaskUrl:
              (reward as any).tintMaskUrl || catalogItem?.tintMaskUrl,
            shadowImageUrl:
              (reward as any).shadowImageUrl || catalogItem?.shadowImageUrl,
            outlineImageUrl:
              (reward as any).outlineImageUrl || catalogItem?.outlineImageUrl,
            overlayImageUrl:
              (reward as any).overlayImageUrl || catalogItem?.overlayImageUrl,
            eyeShadowImageUrl:
              (reward as any).eyeShadowImageUrl ||
              catalogItem?.eyeShadowImageUrl,
            occupiesSlots:
              (reward as any).occupiesSlots || catalogItem?.occupiesSlots,
            rightArmPose:
              (reward as any).rightArmPose || catalogItem?.rightArmPose,
            rightArmImageUrl:
              (reward as any).rightArmImageUrl || catalogItem?.rightArmImageUrl,
            leftArmPose:
              (reward as any).leftArmPose || catalogItem?.leftArmPose,
            leftArmImageUrl:
              (reward as any).leftArmImageUrl || catalogItem?.leftArmImageUrl,
            topBodyImageUrl:
              (reward as any).topBodyImageUrl || catalogItem?.topBodyImageUrl,
            topLeftArmDefaultImageUrl:
              (reward as any).topLeftArmDefaultImageUrl || catalogItem?.topLeftArmDefaultImageUrl,
            topRightArmDefaultImageUrl:
              (reward as any).topRightArmDefaultImageUrl || catalogItem?.topRightArmDefaultImageUrl,
            topRightArmOneHandedImageUrl:
              (reward as any).topRightArmOneHandedImageUrl || catalogItem?.topRightArmOneHandedImageUrl,
            topLeftArmTwoHandedImageUrl:
              (reward as any).topLeftArmTwoHandedImageUrl || catalogItem?.topLeftArmTwoHandedImageUrl,
            topRightArmTwoHandedImageUrl:
              (reward as any).topRightArmTwoHandedImageUrl || catalogItem?.topRightArmTwoHandedImageUrl,
            underTintMaskUrl:
              (reward as any).underTintMaskUrl || catalogItem?.underTintMaskUrl,
            underShadowImageUrl:
              (reward as any).underShadowImageUrl ||
              catalogItem?.underShadowImageUrl,
            underOutlineImageUrl:
              (reward as any).underOutlineImageUrl ||
              catalogItem?.underOutlineImageUrl,
            upperTintMaskUrl:
              (reward as any).upperTintMaskUrl || catalogItem?.upperTintMaskUrl,
            upperShadowImageUrl:
              (reward as any).upperShadowImageUrl ||
              catalogItem?.upperShadowImageUrl,
            upperOutlineImageUrl:
              (reward as any).upperOutlineImageUrl ||
    ...
    const currentItems = dedupeInventoryItems(
      Array.isArray(studentRow.items) ? studentRow.items : [],
    );
    ...
    if (
      ...
      // 일부 기기/네트워크 지연으로 보상 화면이 반복 표시되는 것을 막기 위한 안전장치입니다.
...
export async function markSupabaseRewardUnavailableAsClaimed(
  quizId: string,
  studentId: string,
  attemptNumber?: number,
) {
  try {
    ...
    console.error(
      "[Supabase] markSupabaseRewardUnavailableAsClaimed exception:",
      err,
    );
    return { success: false, error: err.message };
  }
...
// 4. 실시간 세션 관련
export async function getSupabaseLiveSession(
  classCode: string,
  quizId: string,
) {
  try {
...
export async function updateSupabaseLiveSession(
  classCode: string,
  quizId: string,
  updates: any,
) {
...
export async function getPendingReviewResults(classCode: string) {
  try {
    ...
    console.log(
      "[Supabase] getPendingReviewResults success:",
      data?.length || 0,
      "results",
    );
...
export async function gradeSupabaseEssayAnswer(
  ...
  try {
    ...
    if (teacherScore < 0 || teacherScore > maxPoints) {
      return {
        success: false,
        error: `점수는 0~${maxPoints} 사이여야 합니다.`,
      };
    }
    ...
    const newScore = updatedAnswers.reduce(
      (sum, a) => sum + Number(a.earnedPoints || 0),
      0,
    );
    const newPendingScore = updatedAnswers
      ...
      .reduce((sum, a) => {
        const q = questions.find(
          (question: any) => question.id === a.questionId,
        );
    ...
    console.log("[Supabase] gradeSupabaseEssayAnswer:", {
      resultId,
      questionId,
      teacherScore,
      newGradingStatus,
