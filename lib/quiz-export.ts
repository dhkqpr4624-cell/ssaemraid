...
export function exportQuizToJSON(quiz: Quiz): QuizExportData {
  return {
    metadata: {
      ...
      curriculum: (quiz as any).curriculum,
    },
    curriculum: (quiz as any).curriculum,
    settings: {
      ...
      maxRetryAttempts: (quiz as any).maxRetryAttempts || (quiz as any).retryOptions?.maxAttempts || 1,
...
function subjectLabel(subject?: string): string {
  ...
  if (subject === 'science') return '과학';
  if (subject === 'social') return '사회';
  if (subject === 'practical') return '실과';
  return subject || '';
}
...
교육과정: ${data.metadata.curriculum ? `${data.metadata.curriculum.grade || ''}학년 ${data.metadata.curriculum.semester || ''}학기 ${subjectLabel(data.metadata.curriculum.subject)} ${data.metadata.curriculum.unit || ''}단원`.trim() : '(없음)'}
