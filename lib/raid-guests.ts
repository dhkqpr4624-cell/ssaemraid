...
export async function createRaidGuest(roomCode:string,nickname:string,avatarState:AvatarState,items:Item[]):Promise<RaidGuest>{const code=roomCode.toUpperCase(); const existing=await listRaidGuests(code); const g:RaidGuest={id:crypto.randomUUID(),roomCode:code,nickname:nickname.trim(),joinOrder:(existing.reduce((m,x)=>Math.max(m,x.joinOrder),0)+1),avatarState,items,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}; if(enabled){const {data,error}=await supabase.from('raid_guests').insert({id:g.id,room_code:code,nickname:g.nickname,join_order:g.joinOrder,avatar_state:g.avatarState,items:g.items}).select().single(); if(!error)return {...g,id:data.id}; throw new Error(error.message);} localStorage.setItem(key(code),JSON.stringify([...existing,g])); return g;}
