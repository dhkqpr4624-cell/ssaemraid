...
export async function listRaidGuests(roomCode:string,force=false):Promise<RaidGuest[]>{
 ...
 const request=(async()=>{
  if(enabled){const {data,error}=await supabase.from('raid_guests').select('id,room_code,nickname,join_order,avatar_state,items,created_at,updated_at').eq('room_code',code).order('join_order'); if(!error){const mapped=(data||[]).map((r:any)=>({id:r.id,roomCode:r.room_code,nickname:r.nickname,joinOrder:r.join_order,avatarState:r.avatar_state,items:r.items||[],createdAt:r.created_at,updatedAt:r.updated_at}));guestCache.set(code,{at:Date.now(),data:mapped});return mapped;}}
  if(typeof window==='undefined')return [];
  const local=JSON.parse(localStorage.getItem(key(code))||'[]');guestCache.set(code,{at:Date.now(),data:local});return local;
 })();
 guestInflight.set(code,request);try{return await request;}finally{guestInflight.delete(code);}
}
