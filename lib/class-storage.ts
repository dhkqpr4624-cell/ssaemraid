...
function normalizeRoomInventoryItem(item: any): any {
  ...
  return {
    ...
    availableDirections:
      ...
      (item.directionImages ? Object.keys(item.directionImages) : undefined),
    defaultDirection:
      item.defaultDirection ||
      item.availableDirections?.[0] ||
      (item.directionImages ? Object.keys(item.directionImages)[0] : undefined),
  };
}
...
function normalizeStoredStudent(student: Student): Student {
  return {
    ...student,
    items: Array.isArray(student.items)
      ? dedupeInventoryItems(student.items)
      : [],
...
function isDuplicateRestrictedInventoryItem(item: any): boolean {
  // 가구(room)는 같은 아이템을 여러 개 배치할 수 있어야 하므로 중복 허용.
  // 아바타에 장착하는 아이템 등 room이 아닌 보상만 중복 방지/정리 대상으로 봅니다.
...
function hasInventoryItem(items: any[] = [], item: any): boolean {
  if (!isDuplicateRestrictedInventoryItem(item)) return false;
  const newKeys = [item?.catalogItemId, item?.itemId, item?.id]
    .filter(Boolean)
    .map(String);
  if (newKeys.length === 0) return false;
  return (items || []).some((owned: any) => {
    if (!isDuplicateRestrictedInventoryItem(owned)) return false;
    const ownedKeys = [owned?.catalogItemId, owned?.itemId, owned?.id]
      .filter(Boolean)
      .map(String);
    return ownedKeys.some((key) => newKeys.includes(key));
  });
}

function persistDedupeInventoryForStudent(studentId: string): Student | null {
  const students = getFromStorage<Student[]>(STORAGE_KEYS.STUDENTS, []);
  const index = students.findIndex((s) => s.id === studentId);
  if (index === -1) return null;
  const before = Array.isArray(students[index].items)
    ? students[index].items
    : [];
  const after = dedupeInventoryItems(before);
  if (after.length !== before.length) {
    students[index] = { ...students[index], items: after };
    saveToStorage(STORAGE_KEYS.STUDENTS, students);
  }
  return normalizeStoredStudent(students[index]);
}
...
export function createClass(
  ...
  if (!className.trim())
    return { success: false, error: "클래스 이름을 입력해주세요." };
...
export function deleteClass(classCode: string): {
  success: boolean;
  error?: string;
} {
  const classroom = findClassByCode(classCode);
  ...
  saveToStorage(
    ...
    classrooms.filter((c) => c.code !== classCode),
  );

  const students = getFromStorage<Student[]>(STORAGE_KEYS.STUDENTS, []);
  const deletedStudentIds = new Set(
    students.filter((s) => s.classId === classroom.id).map((s) => s.id),
  );
  saveToStorage(
    STORAGE_KEYS.STUDENTS,
    students.filter((s) => s.classId !== classroom.id),
  );

  const quizzes = getFromStorage<Quiz[]>(STORAGE_KEYS.QUIZZES, []);
  const deletedQuizIds = new Set(
    quizzes
      .filter((q) => q.classCode === classCode || q.classId === classroom.id)
      .map((q) => q.id),
  );
  saveToStorage(
    STORAGE_KEYS.QUIZZES,
    quizzes.filter(
      (q) => q.classCode !== classCode && q.classId !== classroom.id,
    ),
  );

  const results = getFromStorage<StudentQuizResult[]>(
  ...
  saveToStorage(
    ...
    results.filter(
      (r) =>
        !deletedQuizIds.has(r.quizId) && !deletedStudentIds.has(r.studentId),
    ),
  );
  ...
  saveToStorage(
    ...
    logs.filter((l) => l.classId !== classroom.id),
  );

  const sessions = getTeacherClassSessions();
  saveToStorage(
    STORAGE_KEYS.TEACHER_SESSIONS,
    sessions.filter((s) => s.classCode !== classCode),
  );
  ...
  if (typeof window !== "undefined") {
    const recent = JSON.parse(
      localStorage.getItem("recentTeacherClasses") || "[]",
    );
    localStorage.setItem(
      "recentTeacherClasses",
      JSON.stringify(recent.filter((c: any) => c.code !== classCode)),
    );
...
export function addStudentToClass(
  ...
  const targetNumber =
    attendanceNumber && attendanceNumber > 0
      ? attendanceNumber
      : Array.from(
          {
            length: Math.max(
          ...
          (_, i) => i + 1,
        ).find((n) => !usedNumbers.has(n)) || classroom.studentCount + 1;
  ...
  if (index !== -1) {
    classrooms[index].studentCount = Math.max(
      classrooms[index].studentCount,
      targetNumber,
    );
    saveToStorage(STORAGE_KEYS.CLASSROOMS, classrooms);
  }
...
export function deleteStudentFromClass(
  classCode: string,
  attendanceNumber: number,
  ...
  const student = students.find(
    (s) =>
      s.classId === classroom.id && s.attendanceNumber === attendanceNumber,
  );
  ...
  saveToStorage(
    STORAGE_KEYS.ACTIVITY_LOGS,
    logs.filter((l) => l.studentId !== student.id),
  );
...
export function findStudent(
  ...
  const migratedStudent = persistDedupeInventoryForStudent(student.id);
  return {
    success: true,
    student: migratedStudent || normalizeStoredStudent(student),
  };
}
...
export function getStudentsByClassId(classId: string): Student[] {
  ...
  const normalizedStudents = students.map((student) => {
    ...
    const before = Array.isArray(student.items) ? student.items : [];
    const after = dedupeInventoryItems(before);
    if (after.length !== before.length) {
      changed = true;
      return { ...student, items: after };
    }
    return student;
  });
  if (changed) saveToStorage(STORAGE_KEYS.STUDENTS, normalizedStudents);
...
export function getStudentSession(): StudentSession | null {
  return getFromStorage<StudentSession | null>(
    STORAGE_KEYS.STUDENT_SESSION,
    null,
  );
}
...
export function getTeacherClassSessions(): TeacherClassSession[] {
  return getFromStorage<TeacherClassSession[]>(
    STORAGE_KEYS.TEACHER_SESSIONS,
    [],
  );
}
...
export function createQuiz(input: QuizInput): {
  ...
  const rewardOptions: QuizRewardOption[] = input.rewardOptions.map((opt) => ({
    ...
    itemId: (opt as any).itemId || generateId("item"),
  }));

  const retryRewardOptions: QuizRewardOption[] | undefined =
    input.retryRewardOptions?.map((opt) => ({
      ...opt,
      id: generateId("reward"),
      itemId: (opt as any).itemId || generateId("item"),
    }));
  ...
  const quiz: Quiz = {
    ...
    retryRewardOptions,
    maxRetryAttempts: Math.max(1, Number((input as any).maxRetryAttempts ?? 1)),
    retryOptions: {
      enabled: input.allowRetry ?? false,
      questionMode: input.retryQuestionMode,
      rewardMode: input.retryRewardMode,
      maxAttempts: Math.max(1, Number((input as any).maxRetryAttempts ?? 1)),
    },
    randomPickEnabled: (input as any).randomPickEnabled,
    randomPickCount: (input as any).randomPickCount,
    shuffleQuestionsEnabled: (input as any).shuffleQuestionsEnabled,
    quizMode: (input as any).quizMode || "normal",
    curriculum: (input as any).curriculum,
    grade: (input as any).grade,
    semester: (input as any).semester,
...
export function updateQuiz(
  ...
  if (input.isReviewEnabled !== undefined)
    updated.isReviewEnabled = input.isReviewEnabled;
  if (input.reviewMode !== undefined) updated.reviewMode = input.reviewMode;
  if ((input as any).curriculum !== undefined)
    (updated as any).curriculum = (input as any).curriculum;
  if ((input as any).grade !== undefined)
    (updated as any).grade = (input as any).grade;
  if ((input as any).semester !== undefined)
    (updated as any).semester = (input as any).semester;
  if ((input as any).subject !== undefined)
    (updated as any).subject = (input as any).subject;
  if ((input as any).unit !== undefined)
    (updated as any).unit = (input as any).unit;
  if ((input as any).randomPickEnabled !== undefined)
    (updated as any).randomPickEnabled = (input as any).randomPickEnabled;
  if ((input as any).randomPickCount !== undefined)
    (updated as any).randomPickCount = (input as any).randomPickCount;
  if ((input as any).shuffleQuestionsEnabled !== undefined)
    (updated as any).shuffleQuestionsEnabled = (
      input as any
    ).shuffleQuestionsEnabled;
  if (input.allowRetry !== undefined) updated.allowRetry = input.allowRetry;
  if (input.retryQuestionMode !== undefined)
    updated.retryQuestionMode = input.retryQuestionMode;
  if (input.retryRewardMode !== undefined)
    updated.retryRewardMode = input.retryRewardMode;
  if ((input as any).maxRetryAttempts !== undefined)
    (updated as any).maxRetryAttempts = Math.max(
      1,
      Number((input as any).maxRetryAttempts || 1),
    );
  (updated as any).retryOptions = {
    ...((updated as any).retryOptions || {}),
    enabled: updated.allowRetry,
    questionMode: updated.retryQuestionMode,
    rewardMode: updated.retryRewardMode,
    maxAttempts: getEffectiveMaxRetryAttempts(updated as any),
  };
  if (input.retryRewardOptions !== undefined) {
    updated.retryRewardOptions = input.retryRewardOptions.map((opt) => ({
      ...opt,
      id: generateId("reward"),
      itemId: (opt as any).itemId || generateId("item"),
    }));
  ...
  if (input.questions !== undefined) {
    ...
    updated.totalScore = updated.questions.reduce(
      (sum, q) => sum + q.points,
      0,
    );
  }
  ...
  if (input.rewardOptions !== undefined) {
    updated.rewardOptions = input.rewardOptions.map(
      (opt) =>
        ({
          id: generateId("reward"),
          itemId: (opt as any).itemId || generateId("item"),
          ...
          roomSize: (opt as any).roomSize,
          spriteConfig: (opt as any).spriteConfig,
          directionImages: (opt as any).directionImages,
          availableDirections: (opt as any).availableDirections,
          defaultDirection: (opt as any).defaultDirection,
          footprint: (opt as any).footprint,
...
export function submitQuizAnswers(
  ...
  rawAnswers: Omit<
    StudentAnswer,
    "isAutoGraded" | "earnedPoints" | "needsReview"
  >[],
): {
  success: boolean;
  result?: StudentQuizResult;
  eligibleRewards?: QuizRewardOption[];
  error?: string;
} {
  const quiz = getQuizById(quizId);
  ...
  const gradedAnswers: StudentAnswer[] = quiz.questions.map((question) => {
    const raw = rawAnswers.find((a) => a.questionId === question.id) ?? {
      questionId: question.id,
    };
    return gradeAnswer(question, raw);
  });

  const autoScore = gradedAnswers.reduce(
    (sum, a) => sum + (a.isAutoGraded ? a.earnedPoints : 0),
    0,
  );
  ...
  const gradingStatus: StudentQuizResult["gradingStatus"] = hasEssay
    ? "pending_review"
    : "auto_complete";
  ...
  const maxRetryAttempts = getEffectiveMaxRetryAttempts(quiz as any);
  if (isRetryAttempt && attemptNumber > maxRetryAttempts + 1) {
    return {
      success: false,
      error: `재도전 가능 횟수(${maxRetryAttempts}회)를 모두 사용했습니다.`,
    };
  }
  ...
  if (
    isRetryAttempt &&
    quiz.retryRewardMode === "different" &&
    quiz.retryRewardOptions
  ) {
    finalRewards = quiz.retryRewardOptions.filter(
      (opt) => opt.requiredScore <= score,
    );
...
export function gradeEssayAnswer(
  ...
  if (teacherScore < 0 || teacherScore > question.points) {
    return {
      success: false,
      error: `점수는 0~${question.points} 사이여야 합니다.`,
    };
  }
  ...
  const newGradingStatus: StudentQuizResult["gradingStatus"] = allReviewed
    ? "fully_graded"
    : "pending_review";
...
export function claimQuizReward(
  ...
  if (resultIndex === -1) {
    return {
      success: false,
      error: "퀴즈 결과를 찾을 수 없거나 이미 보상을 받았습니다.",
    };
  }
  ...
  if (result.isRetryAttempt && quiz.retryRewardMode === "none") {
    return {
      success: false,
      error: "이 회차에는 받을 수 있는 보상이 없습니다.",
    };
  }
  if (
    result.isRetryAttempt &&
    quiz.retryRewardMode === "different" &&
    quiz.retryRewardOptions
  ) {
    rewardOption = quiz.retryRewardOptions.find(
      (opt) => opt.id === rewardOptionId,
    );
  } else {
  ...
  if (!rewardOption)
    return { success: false, error: "존재하지 않는 보상 옵션입니다." };
  ...
  const catalogItem = REWARD_FOLDER_ITEMS.find(
    (item) => item.id === rewardOption.itemId,
  );
  const spriteUrl =
    (rewardOption as any).itemSpriteUrl ||
    catalogItem?.imageUrl ||
    rewardOption.itemImageUrl;
  const iconUrl =
    rewardOption.itemImageUrl || catalogItem?.iconUrl || spriteUrl;

  const item: Item & Record<string, any> =
    rewardOption.itemType === "room"
      ...
      : {
          ...
          imageUrl: spriteUrl,
          hairLayerMode:
            (rewardOption as any).hairLayerMode || catalogItem?.hairLayerMode,
          tintMaskUrl:
            (rewardOption as any).tintMaskUrl || catalogItem?.tintMaskUrl,
          shadowImageUrl:
            (rewardOption as any).shadowImageUrl || catalogItem?.shadowImageUrl,
          outlineImageUrl:
            (rewardOption as any).outlineImageUrl ||
            catalogItem?.outlineImageUrl,
          overlayImageUrl:
            (rewardOption as any).overlayImageUrl ||
            catalogItem?.overlayImageUrl,
          eyeShadowImageUrl:
            (rewardOption as any).eyeShadowImageUrl ||
            catalogItem?.eyeShadowImageUrl,
          occupiesSlots:
            (rewardOption as any).occupiesSlots || catalogItem?.occupiesSlots,
          rightArmPose:
            (rewardOption as any).rightArmPose || catalogItem?.rightArmPose,
          rightArmImageUrl:
            (rewardOption as any).rightArmImageUrl || catalogItem?.rightArmImageUrl,
          leftArmPose:
            (rewardOption as any).leftArmPose || catalogItem?.leftArmPose,
          leftArmImageUrl:
            (rewardOption as any).leftArmImageUrl || catalogItem?.leftArmImageUrl,
          topBodyImageUrl:
            (rewardOption as any).topBodyImageUrl || catalogItem?.topBodyImageUrl,
          topLeftArmDefaultImageUrl:
            (rewardOption as any).topLeftArmDefaultImageUrl || catalogItem?.topLeftArmDefaultImageUrl,
          topRightArmDefaultImageUrl:
            (rewardOption as any).topRightArmDefaultImageUrl || catalogItem?.topRightArmDefaultImageUrl,
          topRightArmOneHandedImageUrl:
            (rewardOption as any).topRightArmOneHandedImageUrl || catalogItem?.topRightArmOneHandedImageUrl,
          topLeftArmTwoHandedImageUrl:
            (rewardOption as any).topLeftArmTwoHandedImageUrl || catalogItem?.topLeftArmTwoHandedImageUrl,
          topRightArmTwoHandedImageUrl:
            (rewardOption as any).topRightArmTwoHandedImageUrl || catalogItem?.topRightArmTwoHandedImageUrl,
          underTintMaskUrl:
            (rewardOption as any).underTintMaskUrl ||
            catalogItem?.underTintMaskUrl,
          underShadowImageUrl:
            (rewardOption as any).underShadowImageUrl ||
            catalogItem?.underShadowImageUrl,
          underOutlineImageUrl:
            (rewardOption as any).underOutlineImageUrl ||
            catalogItem?.underOutlineImageUrl,
          upperTintMaskUrl:
            (rewardOption as any).upperTintMaskUrl ||
            catalogItem?.upperTintMaskUrl,
          upperShadowImageUrl:
            (rewardOption as any).upperShadowImageUrl ||
            catalogItem?.upperShadowImageUrl,
          upperOutlineImageUrl:
            (rewardOption as any).upperOutlineImageUrl ||
            catalogItem?.upperOutlineImageUrl,
  ...
  const studentIndex = students.findIndex(
    (s) => s.id === studentResult.student!.id,
  );
  if (studentIndex === -1)
    return { success: false, error: "학생을 찾을 수 없습니다." };

  const currentItems = dedupeInventoryItems(students[studentIndex].items || []);
  students[studentIndex].items = currentItems;

  if (
    hasInventoryItem(currentItems, {
      ...item,
      itemId: rewardOption.itemId,
      catalogItemId: rewardOption.itemId,
    })
    ...
    // 이미 보유한 장착 아이템을 선택한 경우에도 이 회차 보상 선택은 완료 처리합니다.
    // 보상 수령 화면이 반복 표시되는 것을 막기 위한 localStorage 안전장치입니다.
    result.selectedRewardOptionId = rewardOptionId;
    result.rewardClaimedAt = new Date().toISOString();
    result.rewardClaimedPerAttempt = {
      ...(result.rewardClaimedPerAttempt || {}),
      [result.attemptNumber || 1]: true,
    };
    allResults[resultIndex] = result;
    saveToStorage(STORAGE_KEYS.QUIZ_RESULTS, allResults);
    return { success: true, item, data: result };
  }
...
export function markQuizRewardUnavailableAsClaimed(
  classCode: string,
  attendanceNumber: number,
  quizId: string,
  attemptNumber?: number,
): { success: boolean; data?: StudentQuizResult; error?: string } {
  const allResults = getAllQuizResults();
  const matchingIndexes = allResults
    .map((result, index) => ({ result, index }))
    .filter(
      ({ result }) =>
        result.quizId === quizId &&
        result.classCode === classCode &&
        result.attendanceNumber === attendanceNumber &&
        (!attemptNumber || (result.attemptNumber || 1) === attemptNumber),
    )
    .sort((a, b) => {
      const attemptDiff =
        (b.result.attemptNumber || 1) - (a.result.attemptNumber || 1);
  ...
  if (!target) {
    return { success: false, error: "퀴즈 결과를 찾을 수 없습니다." };
  }

  const result = target.result;
  const currentAttempt = result.attemptNumber || 1;
  const updatedResult: StudentQuizResult = {
    ...result,
  ...
  allResults[target.index] = updatedResult;
  saveToStorage(STORAGE_KEYS.QUIZ_RESULTS, allResults);

  return { success: true, data: updatedResult };
}

export function getPendingReviewResults(
  classCode: string,
): StudentQuizResult[] {
  const allResults = getAllQuizResults();
...
export function getStudentById(
  ...
  const students = getFromStorage<Student[]>(STORAGE_KEYS.STUDENTS, []);
  return (
    students.find((s) => s.id === studentId && s.classId === classroom.id) ??
    null
  );
}
...
export function getStudentQuizResult(
  quizId: string,
  studentId: string,
