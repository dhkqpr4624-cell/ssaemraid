...
function normalizeStudent(data: any) {
  ...
  const normalizedItems = ensureDefaultAvatarItems(data.items || []).map(normalizeRoomInventoryItem);
  return {
    id: data.id,
...
export async function findClassByCode(code: string): Promise<ClassRoom | null> {
  if (isSupabaseEnabled) {
    const data = await supabaseDB.getSupabaseClass(code);
    if (!data) return null;
    return {
      id: data.id,
  ...
  return localStorageDB.findClassByCode(code);
}
...
export async function findStudent(classCode: string, attendanceNumber: number) {
  if (isSupabaseEnabled) {
    ...
    return { 
      success: true, 
      student: normalizeStudent(data) as any
    };
  }
...
export async function getStudentsByClassCode(classCode: string) {
  if (isSupabaseEnabled) {
    try {
      ...
      (data || []).forEach((s: any) => studentMap.set(s.attendance_number, s));
...
export async function submitQuizResult(result: StudentQuizResult) {
  ...
  if (typeof window !== 'undefined') {
    ...
    const filtered = allResults.filter(r => !(r.quizId === result.quizId && r.studentId === result.studentId && (r.attemptNumber || 1) === (result.attemptNumber || 1)));
    localStorage.setItem('quiz_results', JSON.stringify([...filtered, result]));
  }
...
export async function authenticateTeacher(classCode: string, password: string): Promise<{ success: boolean; error?: string }> {
  if (isSupabaseEnabled) {
    try {
      const classroom = await supabaseDB.getSupabaseClass(classCode);
      if (!classroom) {
  ...
  // localStorage 기반 인증
  return localStorageDB.authenticateTeacher(classCode, password);
}
